...
<FORM name='frm' method='POST' action='${pageContext.request.contextPath }/member/login'>
  ...
  <input type="hidden" name="ino" value="${param.ino}"> <%--식별자값 --%>
  <input type="hidden" name="no" value="${param.no}"> <%--bbsno의 값--%>
  
  
  <TABLE>
    <TR>
      <TH>아이디</TH>
      <TD class="pw"><input type="text" name="id" value='${c_id_val }'>
      <c:choose>
      <c:when test="${c_id=='Y'}">
    ...
    <TR>
      <TH>비밀번호</TH>
      <TD class="pw"><input type="password" name="passwd"></TD>
    </TR>
  </TABLE>
