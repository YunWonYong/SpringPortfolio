...
//	    str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
	    ...
	    if (nowGrp >= 2){ 
	      str.append("<ul class='pager'><li><A href='./list.jsp?col="+col+"&word="+word+"&nowPage="+_nowPage+"'>이전</A></li></ul>"); 
	    } 
	    ...
	    if (nowGrp < totalGrp){ 
	      str.append("<ul class='pager'><li><A href='./list.jsp?col="+col+"&word="+word+"&nowPage="+_nowPage+"'>다음</A></li></ul>"); 
	    } 
	    str.append("</DIV>"); 
            ...
            if( fileSize > 0 ) { // 파일이 존재한다면 
                ...
                if ( oldfile.exists()){ 
                    for(int k=0; true; k++){ 
                        ...
                        //파일명으로 조합
                      	String[] oldfile1=originalFileName.split("[.]");
                        oldfile = new File(basePath,oldfile1[0]+"("+k+")."+oldfile1[1]); 
                        ...
                        if(!oldfile.exists()){ //존재하지 않는 경우 
                            ...
                            fileName = oldfile1[0]+"("+k+")."+oldfile1[1]; 
...
//		    str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
		    ...
		    for(int i=startPage; i<=endPage; i++){ 
		      ...
		      if (morePage ==i){ //현재보고있는 페이지가 같을때
		    	  str.append("<span class='span_box_1'><A href='./list?col="+col+"&word="+word+"&nowPage="+i+"'>"+"다음"+"</A></span>");
	 ...
	 public static String paging5(int totalRecord, int nowPage, int recordPerPage, String col, String word){ 
		    int pagePerBlock = 10; // 블럭당 페이지 수 
		    int morePage = nowPage + 1;
		    int totalPage = (int)(Math.ceil((double)totalRecord/recordPerPage)); // 전체 페이지  
		    int totalGrp = (int)(Math.ceil((double)totalPage/pagePerBlock));// 전체 그룹 
...
//		    str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
		    ...
		    int _nowPage = nowPage-1;
		    if (_nowPage != 0 ){ 
			      str.append("<span class='span_box_1'><A href='./paging?col="+col+"&word="+word+"&nowPage="+_nowPage+"'>이전</A></span>"); 
			    } 
		    ...
		    for(int i=startPage; i<=endPage; i++){ 
		      ...
		      if (morePage ==i){ //현재보고있는 페이지가 같을때
		    	  str.append("<span class='span_box_1'><A href='./paging?col="+col+"&word="+word+"&nowPage="+i+"'>"+"다음"+"</A></span>");
//		    	  str.append("<input type='button' value='더보기' class='test'>"+"</input>");
//		    	  str.append("<button type='button' onclick='loadDoc()'>"+"더보기"+"</button>");
		      }
		    } 
